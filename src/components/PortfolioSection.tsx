...
export const PortfolioSection: React.FC<PortfolioSectionProps> = ({ currentLang }) => {
  ...
  // Modal interactive view mode inside lightbox
  const [activeModalTab, setActiveModalTab] = useState<'3d' | 'floorplan' | 'realization' | 'gallery'>('3d');
  ...
  const openProjectModal = (project: Project) => {
    ...
    if (project.visualizationImage) {
      setActiveModalTab('3d');
    } else if (project.floorplanImage) {
      setActiveModalTab('floorplan');
    } else {
      setActiveModalTab('realization');
    }
  };
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-8 pt-4">
                  <div className="md:col-span-7 space-y-4">
                    <h4 className="font-serif text-xl font-normal text-[#1C1B1A]">
                      {t.aboutIntro.slice(0, 0)} Popis projektu
                    </h4>
